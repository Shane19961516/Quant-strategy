...
1. **需要止损。** 现书把强平当止损，10x 逐仓大约逆向 9.6% 才走，训练里独立突破的 MAE 经常先碰到这条线。合理先验是海龟 **2N / 2% 权益 ≈ 2% 价格 ≈ 0.2×投入**，必须严于 9.6%，否则止损就是强平。训练里日线 2N 中位大约在 4–7% 价格，仍在强平之内。
2. **5×投入不是合理止盈。** 那是 50% 价格、相对 2% 止损是 25R。12h 通道上训练独立突破很少把 MFE 推到 5×；海龟本身也没有固定止盈。
3. **若必须写一个固定止盈倍数：** 不要高于 **1×投入（10% 价格，约 5R vs 2% 止损）**；更干净的是 **0** 固定止盈，改用 72 根反向通道作为赢的出场。72 根出场的已实现投入倍数看上表，通常远小于 5。
