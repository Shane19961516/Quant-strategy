...
## 入库裁决
- **结论**: 通过
- **IC均值**: 0.06635900696213733
- **ICIR**: 0.5882884118518708
- **分层价差 (Top-Bottom)**: 0.013165551011111895
- **多空夏普**: 0.922325239147297
- **多空年化**: 0.1173343445942232
- **最大回撤**: -0.2251430551886997

### 门禁明细
| 类别 | 规则 | 结果 | 取值 | 阈值 | 说明 |
|------|------|------|------|------|------|
| sample | `min_months` | PASS | 119.0 | 36 | 样本：最少有效月份数 |
| validity | `min_abs_ic` | PASS | 0.06635900696213733 | 0.02 | 有效性：|IC均值| 下限 |
| validity | `min_abs_icir` | PASS | 0.5882884118518708 | 0.3 | 有效性：|ICIR| 下限 |
| validity | `min_ic_tstat` | PASS | 6.417468925268306 | 2.0 | 有效性：|IC t统计量| 下限 |
| validity | `min_ic_hit_rate` | PASS | 0.7142857142857143 | 0.55 | 有效性：方向校正后 IC 胜率下限 |
...
| layered | `min_abs_q_spread` | PASS | 0.013165551011111895 | 0.003 | 分层：|Top-Bottom 月均价差| 下限 |
| long_short | `min_ls_sharpe` | PASS | 0.922325239147297 | 0.3 | 多空：净夏普下限（含成本） |
| long_short | `min_ls_cagr` | PASS | 0.1173343445942232 | 0.0 | 多空：年化收益下限 |
| long_short | `max_ls_drawdown` | PASS | -0.2251430551886997 | -0.5 | 多空：最大回撤下限（更差则拒） |
| long_short | `max_avg_turnover` | PASS | 1.5971902421723554 | 1.8 | 多空：平均单边换手上限 |
...
panel = store.load_panel("rev_1")         # DataFrame: stocks x dates
...
## 源码入口
- 生成函数: `FACTOR_REGISTRY['rev_1']` @ `factor_engineering.factors`
- 注册表键: `rev_1`
