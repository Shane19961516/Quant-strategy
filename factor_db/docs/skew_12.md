...
- **公式**: -skew(r)_{12m}，滞后1期
- **处理**: 滞后1期 → 1%缩尾 → 行业中性 → 截面 z-score

## 入库裁决
- **结论**: 通过
- **IC均值**: 0.02676895437809868
- **ICIR**: 0.5696659943176428
- **分层价差 (Top-Bottom)**: 0.003994670896642916
- **多空夏普**: 0.6813192895880885
- **多空年化**: 0.03255500847324089
- **最大回撤**: -0.07044733316305496

### 门禁明细
| 类别 | 规则 | 结果 | 取值 | 阈值 | 说明 |
|------|------|------|------|------|------|
| sample | `min_months` | PASS | 108.0 | 36 | 样本：最少有效月份数 |
| validity | `min_abs_ic` | PASS | 0.02676895437809868 | 0.02 | 有效性：|IC均值| 下限 |
| validity | `min_abs_icir` | PASS | 0.5696659943176428 | 0.3 | 有效性：|ICIR| 下限 |
| validity | `min_ic_tstat` | PASS | 5.9201426730144044 | 2.0 | 有效性：|IC t统计量| 下限 |
| validity | `min_ic_hit_rate` | PASS | 0.7222222222222222 | 0.55 | 有效性：方向校正后 IC 胜率下限 |
...
| layered | `min_abs_q_spread` | PASS | 0.003994670896642916 | 0.003 | 分层：|Top-Bottom 月均价差| 下限 |
| long_short | `min_ls_sharpe` | PASS | 0.6813192895880885 | 0.3 | 多空：净夏普下限（含成本） |
| long_short | `min_ls_cagr` | PASS | 0.03255500847324089 | 0.0 | 多空：年化收益下限 |
| long_short | `max_ls_drawdown` | PASS | -0.07044733316305496 | -0.5 | 多空：最大回撤下限（更差则拒） |
| long_short | `max_avg_turnover` | PASS | 0.42702052089350623 | 1.8 | 多空：平均单边换手上限 |
...
panel = store.load_panel("skew_12")         # DataFrame: stocks x dates

# 读取固定时点截面（主调用方式）
s = store.get_factor_on("skew_12", "2019-12-31")
print(s.head())

# 查看解释文档
print(store.get_doc("skew_12")[:500])
...
## 源码入口
- 生成函数: `FACTOR_REGISTRY['skew_12']` @ `factor_engineering.factors`
- 注册表键: `skew_12`
