...
panel = store.load_panel("alpha009")         # DataFrame: stocks x dates

# 读取固定时点截面（主调用方式）
s = store.get_factor_on("alpha009", "2019-12-31")
print(s.head())

# 查看解释文档
print(store.get_doc("alpha009")[:500])
...
## 源码入口
- 生成函数: `FACTOR_REGISTRY['alpha009']` @ `factor_engineering.factors`
- 注册表键: `alpha009`
