...
## 入库裁决
- **结论**: 拒绝
- **IC均值**: -0.0003977066100325
- **ICIR**: -0.002902012695523
- **分层价差 (Top-Bottom)**: -4.025819027157545e-05
- **多空夏普**: -0.3478358703652029
- **多空年化**: -0.0435964917901192
- **最大回撤**: -0.5026078205112126

### 门禁明细
| 类别 | 规则 | 结果 | 取值 | 阈值 | 说明 |
|------|------|------|------|------|------|
| sample | `min_months` | PASS | 503.0 | 36 | 样本：最少有效月份数 |
| validity | `min_abs_ic` | FAIL | 0.0003977066100325 | 0.005 | 有效性：|IC均值| 下限 |
| validity | `min_abs_icir` | FAIL | 0.002902012695523 | 0.06 | 有效性：|ICIR| 下限 |
| validity | `min_ic_tstat` | FAIL | 0.0650853583806932 | 1.5 | 有效性：|IC t统计量| 下限 |
...
| stability | `min_rolling_icir_pos_ratio` | PASS | 0.5625 | 0.5 | 稳定性：滚动 ICIR>0 占比下限 |
| layered | `min_quantile_monotonicity` | PASS | 0.5 | 0.5 | 分层：分位收益单调性下限 |
| layered | `min_abs_q_spread` | FAIL | 4.025819027157545e-05 | 0.0003 | 分层：|Top-Bottom 月均价差| 下限 |
| long_short | `min_ls_sharpe` | FAIL | -0.3478358703652029 | 0.0 | 多空：净夏普下限（含成本） |
| long_short | `min_ls_cagr` | FAIL | -0.0435964917901192 | -0.01 | 多空：年化收益下限 |
| long_short | `max_ls_drawdown` | PASS | -0.5026078205112126 | -0.6 | 多空：最大回撤下限（更差则拒） |
| long_short | `max_avg_turnover` | PASS | 1.445088291492833 | 2.0 | 多空：平均单边换手上限 |
...
panel = store.load_panel("alpha010")         # DataFrame: stocks x dates

# 读取固定时点截面（主调用方式）
s = store.get_factor_on("alpha010", "2019-12-31")
print(s.head())

# 查看解释文档
print(store.get_doc("alpha010")[:500])
...
## 源码入口
- 生成函数: `FACTOR_REGISTRY['alpha010']` @ `factor_engineering.factors`
- 注册表键: `alpha010`
