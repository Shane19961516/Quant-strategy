...
## 入库裁决
- **结论**: 拒绝
- **IC均值**: 0.0048728468776502
- **ICIR**: 0.0827748240979339
- **分层价差 (Top-Bottom)**: 0.000396213547891
- **多空夏普**: -0.3114230103687471
- **多空年化**: -0.0177423101377196
- **最大回撤**: -0.2858860314183379

### 门禁明细
| 类别 | 规则 | 结果 | 取值 | 阈值 | 说明 |
|------|------|------|------|------|------|
| sample | `min_months` | PASS | 503.0 | 36 | 样本：最少有效月份数 |
| validity | `min_abs_ic` | FAIL | 0.0048728468776502 | 0.005 | 有效性：|IC均值| 下限 |
| validity | `min_abs_icir` | PASS | 0.0827748240979339 | 0.06 | 有效性：|ICIR| 下限 |
| validity | `min_ic_tstat` | PASS | 1.8564457349287875 | 1.5 | 有效性：|IC t统计量| 下限 |
...
| stability | `min_rolling_icir_pos_ratio` | PASS | 0.6270833333333333 | 0.5 | 稳定性：滚动 ICIR>0 占比下限 |
| layered | `min_quantile_monotonicity` | PASS | 0.5 | 0.5 | 分层：分位收益单调性下限 |
| layered | `min_abs_q_spread` | PASS | 0.000396213547891 | 0.0003 | 分层：|Top-Bottom 月均价差| 下限 |
| long_short | `min_ls_sharpe` | FAIL | -0.3114230103687471 | 0.0 | 多空：净夏普下限（含成本） |
| long_short | `min_ls_cagr` | FAIL | -0.0177423101377196 | -0.01 | 多空：年化收益下限 |
| long_short | `max_ls_drawdown` | PASS | -0.2858860314183379 | -0.6 | 多空：最大回撤下限（更差则拒） |
| long_short | `max_avg_turnover` | PASS | 1.4470902788803892 | 2.0 | 多空：平均单边换手上限 |
...
panel = store.load_panel("alpha016")         # DataFrame: stocks x dates

# 读取固定时点截面（主调用方式）
s = store.get_factor_on("alpha016", "2019-12-31")
print(s.head())

# 查看解释文档
print(store.get_doc("alpha016")[:500])
...
## 源码入口
- 生成函数: `FACTOR_REGISTRY['alpha016']` @ `factor_engineering.factors`
- 注册表键: `alpha016`
