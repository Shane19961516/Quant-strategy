...
## 入库裁决
- **结论**: 拒绝
- **IC均值**: 0.0107217233046499
- **ICIR**: 0.0710591877024484
- **分层价差 (Top-Bottom)**: 0.0007248617980363
- **多空夏普**: 0.0686101433707721
- **多空年化**: 0.0009037078157294
- **最大回撤**: -0.456258434114356

### 门禁明细
| 类别 | 规则 | 结果 | 取值 | 阈值 | 说明 |
|------|------|------|------|------|------|
| sample | `min_months` | PASS | 478.0 | 36 | 样本：最少有效月份数 |
| validity | `min_abs_ic` | PASS | 0.0107217233046499 | 0.005 | 有效性：|IC均值| 下限 |
| validity | `min_abs_icir` | PASS | 0.0710591877024484 | 0.06 | 有效性：|ICIR| 下限 |
| validity | `min_ic_tstat` | PASS | 1.5535820219780492 | 1.5 | 有效性：|IC t统计量| 下限 |
| validity | `min_ic_hit_rate` | FAIL | nan | 0.51 | 有效性：方向校正后 IC 胜率下限 |
| stability | `min_subperiod_sign_ratio` | PASS | 0.6 | 0.55 | 稳定性：年度子样本 IC 同号占比下限 |
| stability | `half_sample_sign_match` | FAIL | False | True | 稳定性：前后半样本 IC 必须同号 |
| stability | `min_rolling_icir_pos_ratio` | PASS | 0.6307692307692307 | 0.5 | 稳定性：滚动 ICIR>0 占比下限 |
| layered | `min_quantile_monotonicity` | PASS | 0.75 | 0.5 | 分层：分位收益单调性下限 |
| layered | `min_abs_q_spread` | PASS | 0.0007248617980363 | 0.0003 | 分层：|Top-Bottom 月均价差| 下限 |
| long_short | `min_ls_sharpe` | PASS | 0.0686101433707721 | 0.0 | 多空：净夏普下限（含成本） |
| long_short | `min_ls_cagr` | PASS | 0.0009037078157294 | -0.01 | 多空：年化收益下限 |
| long_short | `max_ls_drawdown` | PASS | -0.456258434114356 | -0.6 | 多空：最大回撤下限（更差则拒） |
| long_short | `max_avg_turnover` | PASS | 1.11562334773284 | 2.0 | 多空：平均单边换手上限 |
...
panel = store.load_panel("alpha019")         # DataFrame: stocks x dates

# 读取固定时点截面（主调用方式）
s = store.get_factor_on("alpha019", "2019-12-31")
print(s.head())

# 查看解释文档
print(store.get_doc("alpha019")[:500])
...
## 源码入口
- 生成函数: `FACTOR_REGISTRY['alpha019']` @ `factor_engineering.factors`
- 注册表键: `alpha019`
