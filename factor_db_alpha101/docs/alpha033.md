...
## 入库裁决
- **结论**: 拒绝
- **IC均值**: 0.0037840631996738
- **ICIR**: 0.024744049289734
- **分层价差 (Top-Bottom)**: 5.207553820840878e-06
- **多空夏普**: -0.2925503688641582
- **多空年化**: -0.0467101890180032
- **最大回撤**: -0.5573976155278078

### 门禁明细
| 类别 | 规则 | 结果 | 取值 | 阈值 | 说明 |
|------|------|------|------|------|------|
| sample | `min_months` | PASS | 503.0 | 36 | 样本：最少有效月份数 |
| validity | `min_abs_ic` | FAIL | 0.0037840631996738 | 0.005 | 有效性：|IC均值| 下限 |
| validity | `min_abs_icir` | FAIL | 0.024744049289734 | 0.06 | 有效性：|ICIR| 下限 |
| validity | `min_ic_tstat` | FAIL | 0.5549511614116629 | 1.5 | 有效性：|IC t统计量| 下限 |
...
| stability | `min_rolling_icir_pos_ratio` | PASS | 0.5979166666666667 | 0.5 | 稳定性：滚动 ICIR>0 占比下限 |
| layered | `min_quantile_monotonicity` | PASS | 0.75 | 0.5 | 分层：分位收益单调性下限 |
| layered | `min_abs_q_spread` | FAIL | 5.207553820840878e-06 | 0.0003 | 分层：|Top-Bottom 月均价差| 下限 |
| long_short | `min_ls_sharpe` | FAIL | -0.2925503688641582 | 0.0 | 多空：净夏普下限（含成本） |
| long_short | `min_ls_cagr` | FAIL | -0.0467101890180032 | -0.01 | 多空：年化收益下限 |
| long_short | `max_ls_drawdown` | PASS | -0.5573976155278078 | -0.6 | 多空：最大回撤下限（更差则拒） |
| long_short | `max_avg_turnover` | PASS | 1.5552799941725048 | 2.0 | 多空：平均单边换手上限 |
...
panel = store.load_panel("alpha033")         # DataFrame: stocks x dates

# 读取固定时点截面（主调用方式）
s = store.get_factor_on("alpha033", "2019-12-31")
print(s.head())

# 查看解释文档
print(store.get_doc("alpha033")[:500])
...
## 源码入口
- 生成函数: `FACTOR_REGISTRY['alpha033']` @ `factor_engineering.factors`
- 注册表键: `alpha033`
