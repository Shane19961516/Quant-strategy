...
## 入库裁决
- **结论**: 拒绝
- **IC均值**: -0.0008378554097106
- **ICIR**: -0.0063436716148946
- **分层价差 (Top-Bottom)**: 0.0040242920294008
- **多空夏普**: -0.3955069650521192
- **多空年化**: -0.0442856002852964
- **最大回撤**: -0.4526128400059391

### 门禁明细
| 类别 | 规则 | 结果 | 取值 | 阈值 | 说明 |
|------|------|------|------|------|------|
| sample | `min_months` | PASS | 503.0 | 36 | 样本：最少有效月份数 |
| validity | `min_abs_ic` | FAIL | 0.0008378554097106 | 0.005 | 有效性：|IC均值| 下限 |
| validity | `min_abs_icir` | FAIL | 0.0063436716148946 | 0.06 | 有效性：|ICIR| 下限 |
| validity | `min_ic_tstat` | FAIL | 0.1422737195953026 | 1.5 | 有效性：|IC t统计量| 下限 |
...
| stability | `min_rolling_icir_pos_ratio` | PASS | 0.5083333333333333 | 0.5 | 稳定性：滚动 ICIR>0 占比下限 |
| layered | `min_quantile_monotonicity` | PASS | 0.5 | 0.5 | 分层：分位收益单调性下限 |
| layered | `min_abs_q_spread` | PASS | 0.0040242920294008 | 0.0003 | 分层：|Top-Bottom 月均价差| 下限 |
| long_short | `min_ls_sharpe` | FAIL | -0.3955069650521192 | 0.0 | 多空：净夏普下限（含成本） |
| long_short | `min_ls_cagr` | FAIL | -0.0442856002852964 | -0.01 | 多空：年化收益下限 |
| long_short | `max_ls_drawdown` | PASS | -0.4526128400059391 | -0.6 | 多空：最大回撤下限（更差则拒） |
| long_short | `max_avg_turnover` | PASS | 1.400829173636523 | 2.0 | 多空：平均单边换手上限 |
...
panel = store.load_panel("alpha046")         # DataFrame: stocks x dates

# 读取固定时点截面（主调用方式）
s = store.get_factor_on("alpha046", "2019-12-31")
print(s.head())

# 查看解释文档
print(store.get_doc("alpha046")[:500])
...
## 源码入口
- 生成函数: `FACTOR_REGISTRY['alpha046']` @ `factor_engineering.factors`
- 注册表键: `alpha046`
